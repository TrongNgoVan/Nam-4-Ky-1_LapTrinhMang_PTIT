...
//    }
//}
// Đặt lớp SortingGame trong một file riêng hoặc giữ nó như một lớp nội bộ trong GameView
public class SortingGame extends JPanel {
    private JPanel[] rows;
    private DraggableNumber[][] numbers;
    private int selectedRow = -1;
    private int selectedCol = -1;
    ...
    // Hàm in ra dãy đã sắp xếp và hiển thị lên JTextArea
    private void printSortedArrays() {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 4; i++) {
            sb.append("Dãy ").append(i + 1).append(": ");
            for (int j = 0; j < 5; j++) {
                sb.append(numbers[i][j].getText()).append(" ");
            }
            sb.append("\n");
        }
    ...
    private void setupDragAndDrop(DraggableNumber label, int row, int col) {
        label.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                // Reset màu nền tất cả các labels
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < 5; j++) {
                        numbers[i][j].setBackground(new Color(180, 0, 0));
                        numbers[i][j].setBorder(null);
                    }
                }
        ...
        label.addMouseMotionListener(new MouseMotionAdapter() {
            ...
            public void mouseDragged(MouseEvent e) {
                ...
                // Highlight mục tiêu nếu trong vùng
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < 5; j++) {
                        DraggableNumber targetLabel = numbers[i][j];
                        Point targetPoint = targetLabel.getLocationOnScreen();
                        ...
                        if (targetBounds.contains(p) && (i != selectedRow || j != selectedCol)) {
                            targetLabel.setBackground(new Color(255, 140, 0)); // Highlight target
                        }
                    }
                }
            }
